...
The base prompt defines the LLM's role, task, and field-by-field extraction rules.
Department-specific prompts extend this with domain context.
...
# ── Clarification Round Instructions ─────────────────────────────

CLARIFICATION_CONTEXT = """## Updated Analysis (Clarification Round {round_number})
...
RAG_CONTEXT = """## Reference: Similar Past Projects

Our internal database found the following similar projects that may provide useful context for your analysis. Use them as reference points for technique recommendations and risk assessment, but do NOT copy their assessments — analyze the current request independently.
...
def build_system_prompt(
    ...
    """Assembles the full system prompt from base + department context + optional RAG/clarification blocks."""
    
    parts = [SYSTEM_ROLE, EXTRACTION_RULES, department_context]
    ...
    if similar_projects:
        ...
        parts.append(RAG_CONTEXT.format(similar_projects_text=projects_text))
    ...
    if clarification_round > 0 and clarification_questions and clarification_answers:
        ...
        for q, a in zip(clarification_questions, clarification_answers):
            qa_pairs += f"\n- **Q:** {q}\n  **A:** {a}\n"
        parts.append(CLARIFICATION_CONTEXT.format(
